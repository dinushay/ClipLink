    ...
    for entry in all_streamers:
        ...
        if clip is None and entry.get("last_clip_id") is None:
            ...
            if clip_id != entry.get("last_clip_id"):
                ...
                view.add_item(nextcord.ui.Button(emoji="▶️", label="View Clip", style=nextcord.ButtonStyle.link, url=clip['url']))
                ...
                if clip.get("video_id") and clip.get("vod_offset") is not None:
                    ...
                    view.add_item(nextcord.ui.Button(emoji="📼", label="Go to VOD", style=nextcord.ButtonStyle.link, url=vod_url))
